...
test("Sokrat MOTD Script", async (t) => {
    ...
    await t.test("Normal execution with all services active and dongles", async () => {
        ...
        assert.ok(stdout.includes("Sokrat Service:"), "Missing Sokrat Service label");
        assert.ok(stdout.includes("\x1b[1;32m● ACTIVE"), "Missing active service status");
        assert.ok(stdout.includes("Asterisk Core:"), "Missing Asterisk Core label");
        assert.ok(stdout.includes("\x1b[1;32m● Asterisk 18.19.0"), "Missing asterisk active status");
        assert.ok(stdout.includes("Active Calls:"), "Missing Active Calls label");
        assert.ok(stdout.includes("\x1b[1;31m2"), "Missing asterisk calls in red");
        assert.ok(stdout.includes("System Load:"), "Missing system load label");
        assert.ok(stdout.includes("Memory (RAM):"), "Missing memory label");
        assert.ok(stdout.includes("Root Disk (/):"), "Missing root disk label");
    ...
    await t.test("Offline Asterisk and inactive systemctl", async () => {
        ...
        assert.ok(stdout.includes("\x1b[1;31m○ INACTIVE"), "Missing inactive service status");
        assert.ok(stdout.includes("\x1b[1;31m○ OFFLINE"), "Missing asterisk offline status");
