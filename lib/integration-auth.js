...
async function createEmbedTicket(db, client, params) {
    const { crmUserId, crmUserName, supervisorExtension, requestedScopes } = params;
    ...
    await db.query(
        `INSERT INTO \`asterisk\`.\`dashboard_crm_embed_tickets\`
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [ticketHash, client.client_id, crmUserId, crmUserName, supervisorExtension || null, JSON.stringify(effectiveScopes), expiresAt]
    );
...
/**
 * Consume embed ticket and generate short-lived embed session token atomically
 * @param {object} db
 * @param {string} rawTicket
 * @returns {Promise<{ sessionToken: string, ticket: object }|null>}
 */
async function consumeEmbedTicket(db, rawTicket) {
    ...
    const sessionExpiresAt = new Date(Date.now() + 30 * 60 * 1000); // 30 mins TTL
